...
bool sameGroupMeans(std::vector<std::vector<T>>& treatments) {
    ...
    // own group mean. Then sum it all together. Its degrees of freedom is number of groups - 1. To
    // get mean square of within divide SSW by its degrees of freedom.
    double SSW{};
    ...
    size_t withinDF = treatments.size() - 1;
    ...
    // from the grand mean. It has degrees of freedom of n - k, where n is total samples in
    // population and k is number of groups. MSB (mean square between) = SSB / DF.
    ...
    for (const std::vector<T>& treatment : treatments) {
        double treatmentMean =
            std::accumulate(treatment.begin(), treatment.end(), 0.0,
                            [](double sum, T observation) { return sum + observation; }) /
            treatment.size();
        SSB += std::pow(treatmentMean - grandMean, 2);
    }
    size_t betweenDF = observations.size() - treatments.size();
